# Project lessons / 项目经验

## English

- The supplied fiber background is mandatory. Verify full image decoding in the production browser; valid dimensions alone do not prove a complete image download.

- When asked to enrich an existing animation, preserve its artwork, brightness, geometry, and motion. Add particles as a secondary layer; do not replace the composition with a new circuit aesthetic.

- **Language contract:** The user requires English code comments and complete commit messages across all projects under `opensource/`. Documentation must have equivalent English and Simplified Chinese versions. This overrides the earlier Chinese-only artifact rule; conversation and localized product UI may remain Chinese. Enforce the rules with versioned guidance, staged-content/message hooks, and CI; verify translations manually.
- **Independent navigation:** Product, Docs, Enterprise, Resources, and Community each require a distinct URL, relevant content, and a current-page state. Homepage anchors cannot replace these pages.
- **MaaS narrative:** Lead with the open-source platform slogan and flowing light. Then explain multi-provider access, employee Token FinOps, and commercial intelligent model routing. State planned and contractual delivery boundaries; never invent usage or performance data.
- **Brand and color:** Use the official Api·Go Logo unchanged, preserving transparency, proportions, and colors. Keep neutral surfaces and concentrate the Logo's blue, purple, and orange in meaningful visual accents. Inspect reference sites before borrowing design principles.
- **Immersive composition:** Organize content through continuous space, typography, alignment, and separators. Avoid decorative card stacks. Retain boundaries when they communicate real structure, such as code, forms, comparison tables, and architecture components.
- **Material and motion verification:** Inspect computed backdrop filters and actual production screenshots; source CSS alone does not prove glass effects work. Verify visible animation, pause controls, reduced motion, mobile layouts, and keyboard behavior.
- **Verified publication:** Verify both `/` and `/website/`, workflow completion, remote commit identity, and served artifact bytes. A local build or successful push alone does not prove the public site is updated. History replacement requires explicit authorization and an exact remote lease.

## 中文

- 用户提供的光纤背景必须保留。生产验收须确认图片完整解码，正确尺寸不能证明图片下载完整。

- 为现有动效增加细节时，保留原图、亮度、几何与运动方式。粒子只作为附加层，不能擅自替换成电路风格。

- **语言硬约束：** 用户要求 `opensource/` 下所有项目的代码注释与 commit 全文使用英文，文档同时提供等价的英文与简体中文。该要求覆盖此前产物仅中文的规则；对话与本地化产品界面可以保留中文。通过版本化规范、暂存内容/提交信息 hooks 与 CI 执行，并人工核对翻译。
- **独立导航：** 产品、文档、企业、资源、社区各自拥有独立 URL、对应内容与当前页状态，不能用首页锚点替代。
- **MaaS 叙事：** 首屏突出开源平台 slogan 与流动光影，然后介绍多供应商接入、员工 Token FinOps 和商业智能模型路由。明确规划与合同交付边界，不编造使用量或性能数据。
- **品牌与配色：** 原样使用官方 Api·Go Logo，保持透明度、比例与固有颜色。主体采用中性色，将 Logo 的蓝紫橙集中到有意义的局部视觉中；借鉴参考站前须实际查看页面。
- **沉浸式组织：** 通过连续空间、字阶、对齐与分隔线组织内容，减少装饰性卡片层叠。代码、表单、对比表和架构组件等真实结构可保留边界。
- **材质与动画验证：** 检查生产环境的计算后背景模糊及真实截图，不能仅凭 CSS 判断毛玻璃有效。验证可见动画、暂停、减少动画、移动端布局和键盘操作。
- **发布验收：** 同时验证 `/` 与 `/website/`、工作流完成状态、远端提交身份和线上产物字节。本地构建或推送成功不能单独证明网站已更新。历史替换须有明确授权及精确的远端租约。
